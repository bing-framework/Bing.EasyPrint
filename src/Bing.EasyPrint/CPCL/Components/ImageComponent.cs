...
internal class ImageComponent : ComponentMetadata
{
    ...
    {
        using (var image = Image.Load(fileUrl))
        {
    ...
    private static byte[] GetImageData(Image image)
    {
        ...
        try
        {
            image.Mutate(x =>
            {
                ...
                x.BinaryDither(ErrorDither.FloydSteinberg);
            });
            image.SaveAsBmp(dstStream, encoder);
            ...
            for (var i = 0; i < height; i++)
                Array.Copy(dstBuffer, bfOffBits + (height - 1 - i) * rowSize, result, i * rowRealBytesCount,
                    rowRealBytesCount);
            // 结果数据处理
